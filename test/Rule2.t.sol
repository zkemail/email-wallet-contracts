...
contract Rule2Test is Test {
    ...
    function testSimpleSwap() public {
        ...
        startHoax(vm.addr(1));
        wallet.withdrawETH(param.fromAddressString, newETHBalance);
        assertEq(wallet.balanceOfUser(param.fromAddressString, "ETH"), 0);
        wallet.withdrawERC20(param.fromAddressString, "DAI", newDAIBalance);
        assertEq(wallet.balanceOfUser(param.fromAddressString, "DAI"), 0);
        vm.stopPrank();
